...
instead of Gemini 2.5 Flash for all LLM calls (circumvention analysis, FDA search,
and online fallback).
...
Processes NON-BLOCKING patents per drug:
  ...
  Section 4: Circumvention Analysis per Patent Category
             (uses AlloyDB patent chunks + Claude + online fallback)

Changes from original ipd3bq__6_.py:
  - Replaced google.genai / Gemini with anthropic.AnthropicVertex / Claude Sonnet 4.6
  - Removed Google Search grounding (not available in Claude); online fallback uses
    Claude's knowledge directly.
  - call_gemini → call_claude; async Gemini calls → sync Claude calls in executor
  - All scoring, BQ, AlloyDB, and checkpoint logic is IDENTICAL.
  - Output goes to separate BQ tables: Circumvention_Table_Claude, Patent_Thicket_Score_Table_Claude
"""
...
# (Identical to original — only the LLM backend changes)
...
Orange Book, scientific literature, and patent databases (e.g. USPTO, Espacenet)
— perform a full circumvention analysis for this category.
...
def compute_final_score(total_patents: int, combined_total: int,
    ...
    effective_adjusted = max(0, effective_adjusted)
...
def fetch_relevant_chunks(client, drug: str, source_file: str,
    ...
    Only the drug-specific collection is searched — no fallback scan.
    """
...
def call_claude(prompt: str, max_retries: int = 3) -> dict:
    """Call Claude Sonnet 4.6 and parse the JSON response."""
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                raise
            wait = 2.0 * (2 ** attempt)
            print(f"    [Claude] Error ({e}), retry {attempt+1}/{max_retries} in {wait:.1f}s")
            time.sleep(wait)
    return {}
...
def _call_claude_raw(prompt: str, max_retries: int = 3) -> str:
    """Call Claude and return the raw text response (for search-like prompts)."""
    client = _get_claude_client()
    for attempt in range(max_retries + 1):
        try:
            resp = client.messages.create(
...
async def _analyse_one_category(
    ...
    """Analyse a single category using Claude Sonnet 4.6.

    If no AlloyDB chunks are found, falls back to an online-knowledge Claude call.
    """
    ...
    if all_chunks:
        ...
        # Supplement with FDA/literature search via Claude
        ...
        # ── Fallback path: no chunks found ───────────────────────────────────
        print(f"    [INFO] No AlloyDB chunks for '{category}' patents of '{drug_name}' "
              f"— generating circumvention narrative via Claude online knowledge...")
        ...
        try:
            ...
            if online_json:
                ...
                raise ValueError("Empty or unparseable response from online prompt")
        except Exception as e:
            ...
            result = {
                ...
                "regulatory_viability": f"Error: {e}",
                "summary": (
                    "Patent text was unavailable in the local database and the "
                    "Claude analysis also failed. Manual review required."
                ),
            }
...
async def run_circumvention_analysis(
    ...
    t0 = time.time()
    print(f"\n[Circumvention] Analysing design-around strategies for {drug_name} (Claude)...")
    ...
    async def _bounded_analyse(category, patents):
        async with semaphore:
            return await _analyse_one_category(
                drug_name, category, patents, chroma_client,
            )
...
# ── BigQuery Writers ──────────────────────────────────────────────────────────
# Output to SEPARATE tables so Gemini & Claude results coexist
...
def write_circumvention_to_bq(circumvention_by_drug: dict):
    """
    Flattens circumvention results and writes to Circumvention_Table_Claude.
    Identical schema to original Circumvention_Table.
    """
    rows = []
    ...
    job.result()
    print(f"[BQ] {BQ_CIRC_TABLE}: {len(df_circ)} new rows written → {table_ref}")


def write_score_to_bq(drug_scores: list, refresh=False):
    """
    Writes patent thicket scores to Patent_Thicket_Score_Table_Claude.
    """
    rows = []
    ...
    job = client.load_table_from_dataframe(df_score, table_ref, job_config=job_config,
                                           location=BQ_LOCATION)
    job.result()
    print(f"[BQ] {BQ_SCORE_TABLE}: {len(df_score)} new rows written → {table_ref}")
...
def process_patents(skip_circumvention=False, drug_filter=None, refresh_scores=False,
    ...
    # Ensure "No Of Forecasted Patents" column is numeric
    forecasted_col = None
    for c in df.columns:
    ...
    if forecasted_col is not None:
        ...
        print(f"  [WARN] 'No Of Forecasted Patents' column not found.")
    ...
    # ── GCS Checkpoint ────────────────────────────────────────────────────────
    _ipd3_ckpt_subfolder = "ipd3_checkpoints_claude"
    _ipd3_ckpt_file = "completed_drugs_claude.json"
    completed_drugs = set()
    ...
    if refresh_scores or rerun or _force_clear:
        ...
        try:
            ...
            if ckpt_data and isinstance(ckpt_data, list):
                ...
                print(f"[CHECKPOINT] {len(completed_drugs)} drug(s) already done (Claude): {completed_drugs}")
        except Exception as e:
    ...
    if completed_drugs and not skip_circumvention:
        try:
            ...
            if missing:
                print(f"[CHECKPOINT] {len(missing)} drug(s) marked done but missing from BQ — will reprocess: {sorted(missing)}")
                completed_drugs -= missing
        except Exception as e:
            print(f"[CHECKPOINT] Could not verify against BQ tables ({e}) — trusting checkpoint")
    ...
    if not skip_circumvention and processed_drugs:
        print(f"\n{'='*60}")
        print(f"Running circumvention / 505(b)(2) design-around analysis (Claude)...")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Patent Thicket Analysis & Circumvention Strategy (505(b)(2)) — CLAUDE VERSION"
    )
    parser.add_argument(
        "drug", nargs="?", default=None,
        help="Optional: single drug name to process (e.g. Semaglutide).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Delete existing score rows for the drug(s) and recompute.",
    )
    parser.add_argument(
        "--rerun", action="store_true",
        help="Bypass checkpoint and rerun everything.",
    )
    parser.add_argument(
